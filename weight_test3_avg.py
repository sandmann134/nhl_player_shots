...
def main():
    ...
    ledger_name_flat = f'daily_ledger_scaled_2324flat'
    model_table_flat = f'modelled_likelihoods_2324flat'

    ledger_name_x10_flat = f'daily_ledger_scaled_2324flat_x10_3'
    model_table_x10_flat = f'modelled_likelihoods_2324flat_x10_3'

    ledger_x10_flat22 = 'daily_ledger_scaled_222324flat_x10_3'
    model_table_x10_flat22 = 'modelled_likelihoods_222324flat_x10_3' 
    
    ledger_name_ref = f'daily_ledger_scaled_weight4'
    model_table_ref = f'modelled_likelihoods_weight4'

    ledger_name_weight3 = f'daily_ledger_scaled_weight3'

    ledger_names = [ledger_name_flat, ledger_name_x10_flat, ledger_x10_flat22, ledger_name_weight3, ledger_name_ref]

    # CREATE
    create_ledger(ledger_name_flat)
    create_player_models(model_table_flat)

    create_ledger(ledger_name_x10_flat)
    create_player_models(model_table_x10_flat)

    create_ledger(ledger_x10_flat22)
    create_player_models(model_table_x10_flat22)

    # model players with current weights
    fetch_and_store_player_data(1, 1, 1, 0)
    fetch_and_store_player_data(3, 1, 1, 0)
    fetch_and_store_player_data(3, 1, 1, 1)
    fetch_and_store_player_data(4, 3, 2, 1)

    # update correct ledger with actual results
    update_ledger(ledger_name_flat, model_table_flat)
    update_ledger(ledger_name_x10_flat, model_table_x10_flat)
    update_ledger(ledger_x10_flat22, model_table_x10_flat22)
    update_ledger(ledger_name_ref, model_table_ref)
...
#            print(f"CSV file {model_table}_{today_date}.csv created at {print_path}.")
    ...
    for i in range(len(daily_dollar_amounts)):
        plt.plot(daily_dollar_amounts[i])

    plt.xlabel('Day')
    plt.ylabel('Dollar Amount')
    plt.title('Daily Dollar Amounts for Different Weights')
    plt.legend(['x_10=x_24=x_23 = 1, x_22=0', 'x_10=3, x_24=x_23=1, x_22=0', 'x_10=3, x_24=x_23=x_22=1', 'x_10=x_24=3, x_23=2, x_22=1', 'x_10=4, x_24 = 3, x_23=2, x_22=1'])
    plt.savefig(os.path.join(script_dir, 'plots/weight_test_flat.png'))
    #plt.show()
