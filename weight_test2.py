...
def main():
    ...
    for i in range(2, 5):
        ...
        if i == 3:
            ledger_name = f'daily_ledger_scaled_weight4'
            model_table = f'modelled_likelihoods_weight4'
        else:
            ledger_name = f'daily_ledger_scaled_weight4_x24_{str(i)}'
            model_table = f'modelled_likelihoods_weight4_x24_{str(i)}'
        ...
        fetch_and_store_player_data(4, i, 2, 1)
...
#            print(f"CSV file {model_table}_{today_date}.csv created at {print_path}.")
    ...
    plt.legend(['x_24 = 2', 'x_24 = 3 (og)', 'x_24 = 4'])
    plt.savefig(os.path.join(script_dir, 'plots/weight_test_x24.png'))
    plt.show()
